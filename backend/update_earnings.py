...
def safe_get(url):
    try:
        ...
        print("safe_get error:", e, "URL:", url)
        return None
...
def fetch_atr_ratio_alpha(ticker):
    ...
    try:
        ...
        print("fetch_atr_ratio_alpha parse error:", e, "ticker:", ticker)
        return None
...
def fetch_price_alpha(ticker):
    ...
    try:
        ...
        print("fetch_price_alpha parse error:", e, "ticker:", ticker)
        return None
...
def fetch_volatility(ticker, cache):
    ...
    # Only ATR + price (VOLATILITY/BETA removed to reduce rate-limit issues)
    atr = fetch_atr_ratio_alpha(ticker)
    time.sleep(1)
    price = fetch_price_alpha(ticker)
    time.sleep(1)

    # Fallbacks to yesterday's cache
    ...
    # We keep move/beta in cache for compatibility, but don't refetch them
    move = prev.get("move")
    beta = prev.get("beta")
...
def compute_volatility_score(entry):
    ...
    # ATR score (0–100): 2% → 20, 5% → 50, 10% → 100
    atr_score = min(norm_atr * 1000, 100)

    # Optional signals (if you ever populate move/beta again)
...
def merge_sources():
    ...
    # Then merge AlphaVantage with rules:
    # - If Finnhub is near-term, keep Finnhub
    # - Otherwise prefer AlphaVantage
    for row in b:
        ticker = row["ticker"]
        ...
        # Otherwise AlphaVantage overrides
        merged[ticker] = row
